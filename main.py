...
@app.route("/")
def health(): 
    return "ENGINE_V12.6_INSTITUTIONAL_FINAL_RUNNING", 200
...
def connect():
    ...
    while True:
        try:
            print(f"DEBUG: Logging in {EMAIL}")
...
def get_candles(asset):
    try:
        ...
        candles = client.get_candles(asset, 60, end_time, 100)
...
def wait_result(pair, entry_time):
    """Wait for candle to exist up to 75 sec max"""
    max_wait = entry_time + 75
    while int(time.time()) < max_wait:
        df = get_candles(pair)
        if df is None:
            time.sleep(1)
            continue
        candle = df[df["time"]==entry_time]
        if not candle.empty:
...
def process_trade(pair, direction, entry_time):
    ...
    if candle is None:
        print(f"⚠️ {asset_label} candle not ready, skipping MTG-1")
    ...
    if mtg_candle:
        ...
        stats["loss"] +=1
        send_telegram(text=f"❌ <b>{asset_label} MTG VERIFICATION FAIL (LOSS)</b>", sticker=STICKER_LOSS)
...
def signal_loop():
    global trade_active
    last_min = None
    print("🚀 ENGINE V12.6 INSTITUTIONAL STARTED")
