import os, time, pandas as pd, requests, pytz, random, sys
import numpy as np
...
app = Flask(__name__)
VERSION = "V20.0-MTG"
LOCK_FILE = "/tmp/quotex_bot_v20.lock"  # prevents old processes

# ---- Kill/ignore old process if running ----
if os.path.exists(LOCK_FILE): 
    # another instance is already running 
    print("OLD PROCESS DETECTED - EXITING THIS INSTANCE")
    sys.exit(0)
...
@app.route('/') 
def health(): 
    return jsonify(status="online", version=VERSION, mode="4-5 MIN MTG MODE"), 200
...
if __name__ == "__main__": 
    try: 
        ...
        # cleanup lock when container stops 
        if os.path.exists(LOCK_FILE): 
            os.remove(LOCK_FILE)
